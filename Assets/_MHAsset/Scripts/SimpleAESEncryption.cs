	...
	public static class SimpleAESEncryption
	{
		...
		public static byte[] Encode(byte[] data, string password)
		{
			using (var aes = Aes.Create())
			{
				...
				var aesEncryptor = aes.CreateEncryptor();
				var encryptedBytes = aesEncryptor.TransformFinalBlock(data, 0, data.Length);
		...
		/// <summary>
		/// Decrypts a byte array that was encrypted with the Encode method
		/// </summary>
		/// <param name="encryptedData">The encrypted byte array (containing IV + encrypted data)</param>
		/// <param name="password">The password to use when decrypting</param>
		/// <returns>The original byte array</returns>
		public static byte[] Decode(byte[] encryptedData, string password)
		{
			using (var aes = Aes.Create())
			{
				// Extract IV from the beginning of the encrypted data
				var ivLength = aes.IV.Length; // AES IV is typically 16 bytes
				var iv = new byte[ivLength];
				Array.Copy(encryptedData, 0, iv, 0, ivLength);

				// Extract the actual encrypted data
				var encryptedBytes = new byte[encryptedData.Length - ivLength];
				Array.Copy(encryptedData, ivLength, encryptedBytes, 0, encryptedBytes.Length);

				var decryptor = aes.CreateDecryptor(ConvertToKeyBytes(aes, password), iv);
