...
public class AnimationPostprocessor : AssetPostprocessor {
    ...
    void OnPreprocessModel() {
        ...
        // Validate reference importer
        if (referenceImporter == null) {
            Debug.LogError($"[AnimationPostProcessor] Reference FBX or ModelImporter is null. Please assign a valid Reference FBX in AnimationPostProcessorSettings. Skipping: {importer.assetPath}");
            return;
        }
        ...
        if (referenceAvatar == null && referenceImporter != null) {
    ...
    static void LoadSettings() {
        ...
        if (guids.Length > 0) {
            ...
            if (settings != null) {
                ...
                // Validate reference FBX and get its importer
                if (referenceFBX != null) {
                    string referenceFBXPath = AssetDatabase.GetAssetPath(referenceFBX);
                    if (!string.IsNullOrEmpty(referenceFBXPath)) {
                        referenceImporter = AssetImporter.GetAtPath(referenceFBXPath) as ModelImporter;
                        if (referenceImporter == null) {
                            Debug.LogError($"[AnimationPostProcessor] Reference FBX at path '{referenceFBXPath}' is not a valid FBX file or ModelImporter.");
                        }
                    } else {
                        Debug.LogError($"[AnimationPostProcessor] Could not get asset path for Reference FBX. Please ensure the Reference FBX is properly assigned in AnimationPostProcessorSettings.");
                    }
                } else {
                    Debug.LogWarning($"[AnimationPostProcessor] Reference FBX is not assigned in AnimationPostProcessorSettings. Animation processing will be skipped.");
                    referenceImporter = null;
                }
                settingsLoaded = true;
            } else {
                Debug.LogError($"[AnimationPostProcessor] Failed to load AnimationPostProcessorSettings from path: {path}");
                settingsLoaded = false;
            }
        } else {
            Debug.LogWarning("[AnimationPostProcessor] AnimationPostProcessorSettings asset not found. Please create the settings asset.");
            settingsLoaded = false;
        }
    }
}
